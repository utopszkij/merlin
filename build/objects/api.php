...
include __DIR__.'/../config.php';
 ...
 class API {
		...
		public function insert(\RATWEB\DB\Record &$record): string {
			...
			if ($result == '') {
				$result = $this->accessRight('insert',$oldRecord, $record, getSession('logedUser',0));
			}	
			if ($result == '') {
			...
			if ($result == '') {
				...
				if ($result == '') {
					if ($this->updateLog) {
						...
						$logRecord->time = now();
						$logRecord->user_id = $this->userInLog;
						$logRecord->type = 'update';
						$logRecord->info = '{old:'.JSON_encode($oldrecord).
				...
				if ($result == '') {
				   if ($result == '') {
				      if ($this->deleteLog) {
						 ...
						 $logRecord->time = now();
						 $logRecord->user_id = $this->userInLog;
						 $logRecord->type = 'delete';
						 $logRecord->info = JSON_encode($oldRecord);
						 $dbLog->insert($logRecord);
						 $result = $dbLog->error;
				      }
				   }   
		...
		public function upoladFiles(string $fileNamePrefix,
			...
			foreach ($_FILES as $fn => $fv) {
				...
				if (move_uploaded_file($_FILES[$fn]['tmp_name'], $uploadFile)) {
					$url = $uploadDir.'/'.$fileNamePrefix.$fileName;
